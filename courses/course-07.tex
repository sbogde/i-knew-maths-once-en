...
% lecture 7, page 8
\section*{Lecture 7 - Page 8}

\noindent\fbox{T} \ Every group $G$ is isomorphic to a group of permutations.

\bigskip
\noindent\underline{Proof}: \ Let $a\in G$.
...
\]
\[
(\forall)\,y\in G \ \exists\,x\in G \text{ such that } \theta(x)=y. \quad \text{(take } x=a^{-1}y\text{)}
\]
...
\bigskip
\noindent Let $f: G \to (S_G,\circ,1_G)$.
...
\bigskip
\noindent\underline{$f$ is injective}
...
\bigskip
\noindent ker$f=\{e\} \overset{\text{F.I.T}}{\Longrightarrow} \ \text{Im}f = f(G) \leq (S_G,\circ) \ \Big|$
...
\bigskip
\noindent\underline{Ex}: \ a group with 4 elem.\ $(S_4,\circ,1_{S_4})$
...
\bigskip
\noindent Find the corresponding statement for Cayley's Theorem.

\bigskip
\noindent $T_A = \{f \mid f:A\to A\}$

\noindent $(T_A,\circ,1_A) \supseteq (S_A,\circ,1_A)$ \ $\to$ the transformation monoid

\bigskip
\noindent We call a transformation monoid any $N\leq (T_A,\circ,1_A)$.

\bigskip
\noindent\underline{Cayley's Theorem}: \ Every monoid is isomorphic to a transformation monoid.
